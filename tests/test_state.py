...
def test_ershov_state_root_takes_precedence_over_legacy_environment(tmp_path: Path) -> None:
    repo_root = Path(__file__).resolve().parents[1]
    state_root = tmp_path / "ershov-state"
    legacy_root = tmp_path / "legacy-dreaming-state"
    code = """
from pathlib import Path
import sys
sys.path.insert(0, str(Path.cwd() / 'src'))
...
"""
    env = dict(os.environ)
    env["HERMES_ERSHOV_STATE_ROOT"] = str(state_root)
    env["HERMES_DREAMING_STATE_ROOT"] = str(legacy_root)
    ...
    assert result.stdout.strip() == str(state_root)
