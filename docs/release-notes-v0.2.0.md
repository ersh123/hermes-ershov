# Hermes Mnemos v0.2.0 Release Notes

Status: approved for release and shipped as `v0.2.0` on 2026-05-28.

## What changed since v0.1.1

- Review UX got a real decision loop: `mnemos summarize <artifact>`, `mnemos approve <artifact> <proposal-id|all>`, `mnemos reject <artifact> <proposal-id> --reason ...`, and `mnemos review --open` now make the artifact easier to inspect and act on.
...
- Onboarding is finally honest and usable. The repo now includes install/update docs, an offline quickstart, persona examples, and a safety page that spells out what Mnemos can and cannot mutate.
- The first phase-7 slice landed as a real dogfoodable feature: `mnemos report-card` now renders a redacted shareable report card from an artifact and can emit a JSON companion.
...
- The `hermes-mnemos` package name was chosen for the public project line. Re-check PyPI before enabling PyPI upload.
