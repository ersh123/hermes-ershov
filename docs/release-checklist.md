...
- [ ] After a real scheduled run, smoke `COMMIT="$(git -C ~/.hermes/plugins/hermes-ershov rev-parse --short HEAD)" && ershov soak --state-root ~/.hermes/ershov --since-hours 30 --require-timer --require-source systemd --require-commit "$COMMIT"`
